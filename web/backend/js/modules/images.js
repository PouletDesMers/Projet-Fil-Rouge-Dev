        ...
        fetch(`/admin/api/carousel-images/${image.id_image}`, {
          ...
          .then((response) => {
            if (response.ok) {
              ...
              console.warn(
                `Erreur lors de la mise à jour de l'ordre de l'image ${image.id_image}`,
              );
            }
          })
          .catch((error) => {
            console.warn(
              `Erreur lors de la correction de l'ordre de l'image ${image.id_image}:`,
              error,
            );
          }),
      );
...
async function loadImages() {
  try {
    ...
    const errContainer = document.getElementById("imagesContainer");
    if (errContainer)
            ...
            <tr>
              ...
              <td>
                <div class="btn-group" role="group">
                  ${
                    index > 0
                      ? `<button class="btn btn-sm btn-outline-secondary" onclick="AdminImages.moveImage(${image.id_image}, 'up')" title="Déplacer vers le haut">
                  ...
                  ${
                    index < images.length - 1
                      ? `<button class="btn btn-sm btn-outline-secondary" onclick="AdminImages.moveImage(${image.id_image}, 'down')" title="Déplacer vers le bas">
...
function getNextImageOrder() {
  const tableRows = document.querySelectorAll(
    "#imagesContainer table tbody tr",
  );
  let maxOrder = 0;
