...
async function loadRoles() {
  try {
    ...
    if (!roles || roles.length === 0) {
      container.innerHTML =
        '<div class="alert alert-info">Aucun rôle trouvé</div>';
      return;
    }
    ...
    if (rolesContainer)
      rolesContainer.innerHTML = `<div class="alert alert-danger">Erreur: ${error.message}</div>`;
  }
}
...
async function loadPermissions() {
  try {
    ...
    if (!permissions || permissions.length === 0) {
      container.innerHTML =
        '<div class="alert alert-info">Aucune permission disponible</div>';
      return;
    }
    ...
    if (permissionsContainer)
      permissionsContainer.innerHTML = `<div class="alert alert-danger">Erreur: ${error.message}</div>`;
  }
}
...
async function removePermissionFromRole(roleId, permissionCode) {
  try {
    const response = await fetch(
      `/admin/api/roles/${roleId}/permissions/${permissionCode}`,
      {
        method: "DELETE",
        headers: buildRoleHeaders(),
        credentials: "include",
...
async function openPermissions(roleId, roleName) {
  ...
  if (bodyEl)
    bodyEl.innerHTML =
      '<div class="text-muted small">Chargement des permissions...</div>';
  ...
  try {
    ...
    if (!allPermsRes.ok || !rolePermsRes.ok)
      throw new Error("Chargement des permissions impossible");
    ...
    if (bodyEl)
      bodyEl.innerHTML =
        html || '<div class="text-muted">Aucune permission</div>';
    modal.show();
  } catch (error) {
    console.error("Erreur openPermissions:", error);
    if (bodyEl)
      bodyEl.innerHTML = `<div class="text-danger">Erreur: ${error.message}</div>`;
    modal.show();
  }
}
