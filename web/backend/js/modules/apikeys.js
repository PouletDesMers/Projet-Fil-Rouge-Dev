  ...
  async loadAPIKeys() {
    try {
      ...
      if (!keys || keys.length === 0) {
        container.innerHTML =
          '<div class="alert alert-info"><i class="bi bi-info-circle"></i> Aucune clé API trouvée</div>';
        return;
      }
      ...
      const errContainer = document.getElementById("apiKeysContainer");
      if (errContainer)
  ...
  async createKey() {
    try {
      ...
      const permissions = document
        .getElementById("apiKeyPermissions")
        .value.trim();
      ...
      const resultModal = new bootstrap.Modal(
        document.getElementById("apiKeyResultModal"),
      );
      resultModal.show();
  ...
  async toggleKeyStatus(keyId, newStatus) {
    try {
      ...
      AdminUtils.showAlert(
        `Clé ${newStatus ? "activée" : "désactivée"} avec succès`,
        "success",
      );
      this.loadAPIKeys();
    } catch (error) {
