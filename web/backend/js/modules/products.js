...
function renderImagesPreview() {
  ...
  if (productImagesList.length === 0) {
    el.innerHTML =
      '<span class="text-muted small fst-italic">Aucune image pour l\'instant</span>';
    return;
  }
  el.innerHTML = productImagesList
    .map(
      (url, i) => `
...
function addImageUrl() {
  ...
  document.getElementById("productImages").value =
    JSON.stringify(productImagesList);
  input.value = "";
  renderImagesPreview();
}
...
AdminProducts.importProductsFile = async function () {
  ...
  const categoryMode =
    document.getElementById("importProductsCategoryMode")?.value || "file";
  const btn = document.getElementById("confirmImportProductsBtn");

  if (!fileInput || !fileInput.files.length) {
    AdminProducts.showImportAlert(
  ...
  if (categoryMode === "current" && AdminProducts.currentCategoryId) {
    formData.append(
      "forcedCategoryId",
      String(AdminProducts.currentCategoryId),
    );
  }
  ...
  try {
    ...
    if (
      typeof AdminProducts.loadCategoryProducts === "function" &&
      AdminProducts.currentCategoryId
    ) {
        ...
        fetch(`/admin/api/products/${product.id_produit}`, {
          ...
          .then((response) => {
            if (response.ok) {
              ...
              console.warn(
                `Erreur lors de la mise à jour de l'ordre du produit ${product.id_produit}`,
              );
            }
          })
          .catch((error) => {
            console.warn(
              `Erreur lors de la correction de l'ordre du produit ${product.id_produit}:`,
              error,
            );
          }),
      );
              ...
              <tr>
                ...
                <td>
                  <div class="btn-group" role="group">
                    ${
                      index > 0
                        ? `<button class="btn btn-sm btn-outline-secondary" onclick="AdminProducts.moveProduct(${product.id_produit}, 'up')" title="Déplacer vers le haut">
                    ...
                    ${
                      index < products.length - 1
                        ? `<button class="btn btn-sm btn-outline-secondary" onclick="AdminProducts.moveProduct(${product.id_produit}, 'down')" title="Déplacer vers le bas">
...
async function moveProduct(productId, direction) {
  try {
    ...
    AdminUtils.showAlert(
      `Erreur lors du déplacement: ${error.message}`,
      "danger",
    );
  }
}
  ...
  try {
    ...
    if (type === "product" && AdminMain.currentCategoryId()) {
      ...
      items = items.filter(
        (item) =>
          item.id_categorie &&
          item.id_categorie.toString() === categoryId.toString(),
      );
    }
