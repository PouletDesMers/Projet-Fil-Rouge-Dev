...
// ── Cache pour éviter les appels redondants à /admin/api/categories ───
let _categoriesCache = null;
let _categoriesCacheTime = 0;
const CATEGORIES_CACHE_TTL = 30000; // 30 secondes

function _getCachedCategories() {
  if (
    _categoriesCache &&
    Date.now() - _categoriesCacheTime < CATEGORIES_CACHE_TTL
  ) {
...
function _setCategoriesCache(data) {
  _categoriesCache = data;
  _categoriesCacheTime = Date.now();
}

function _invalidateCategoriesCache() {
        ...
        fetch(`/admin/api/categories/${category.id_categorie}`, {
          ...
          .then((response) => {
            if (response.ok) {
              ...
              console.warn(
                `Erreur lors de la mise à jour de l'ordre de la catégorie ${category.id_categorie}`,
              );
            }
          })
          .catch((error) => {
            console.warn(
              `Erreur lors de la correction de l'ordre de la catégorie ${category.id_categorie}:`,
              error,
            );
          }),
      );
...
// Fetch categories from API (avec cache)
async function fetchCategories() {
  ...
  _setCategoriesCache(categories);
  return categories;
}
...
async function loadCategories() {
  try {
    ...
    // Sécurité DOM : abandon propre si le conteneur n'est pas présent dans le DOM
              ...
              <tr>
                ...
                <td class="table-actions">
                  <div class="btn-group" role="group">
                    ${
                      index > 0
                        ? `<button class="btn btn-sm btn-outline-secondary" onclick="AdminCategories.moveCategory(${cat.id_categorie}, 'up')" title="Déplacer vers le haut">
                    ...
                    ${
                      index < categories.length - 1
                        ? `<button class="btn btn-sm btn-outline-secondary" onclick="AdminCategories.moveCategory(${cat.id_categorie}, 'down')" title="Déplacer vers le bas">
    ...
    // Sécurité DOM : vérification null dans le bloc catch également
    const categoriesContainer = document.getElementById("categoriesContainer");
...
function openCategoryModal(category = null) {
  ...
  if (category) {
    ...
    document.getElementById("categoryIcone").value =
      category.icone || "bi bi-tag";
...
// Load categories for product dropdown (utilise le cache)
async function loadCategoriesForProducts() {
  try {
    ...
    try {
      ...
      // En cas d'erreur, tenter un fetch direct (sans cache)
      const token = localStorage.getItem("token");
      const response = await fetch("/admin/api/categories", {
        headers: {
          Authorization: `Bearer ${token}`,
...
async function deleteCategory(categoryId, categoryName) {
  ...
  AdminUtils.showToast(
    "Fonctionnalité de suppression en développement",
    "info",
  );
}
