  ...
  ?.addEventListener("click", async () => {
    ...
    try {
      ...
      showAlert(
        "editUserAlert",
        "success",
        `Nouveau mot de passe: <strong>${newPassword}</strong><br><small>Copiez-le et envoyez-le à l'utilisateur de manière sécurisée.</small>`,
      );
    } catch (error) {
      console.error("Error:", error);
      showAlert("editUserAlert", "danger", error.message);
...
document
  ...
  ?.addEventListener("click", async () => {
    if (
      !confirm(
        "⚠️ ATTENTION : Supprimer définitivement cet utilisateur ? Cette action est irréversible !",
      )
    ) {
      return;
    }
...
function openImageModal(imageId = null) {
  ...
  if (imageId) {
    ...
    // Add mode
    document.getElementById("imageModalTitleText").textContent =
      "Ajouter Image";
  ...
  try {
    ...
    const imageData = {
      ...
      alt_text:
        ...
        document.getElementById("imageTitre").value,
      ordre_affichage:
        parseInt(document.getElementById("imageOrder").value) || 1,
    ...
    const url = isEdit
      ? `/admin/api/carousel-images/${imageId}`
      : "/admin/api/carousel-images";
    ...
    showToast(
      `Image ${isEdit ? "modifiée" : "ajoutée"} avec succès !`,
      "success",
    );
...
async function deleteImage(imageId) {
  if (
    !confirm(
      "⚠️ ATTENTION : Supprimer définitivement cette image ? Cette action est irréversible !",
    )
  ) {
    return;
  }
...
async function toggleImageStatus(imageId, activate) {
  try {
    ...
    showToast(
      `Image ${activate ? "activée" : "désactivée"} avec succès !`,
      "success",
    );
    loadImages();
  } catch (error) {
...
async function changeImageOrder(imageId, direction) {
  try {
    ...
    const targetImage = images.find(
      (img) => img.ordre_affichage === targetOrder,
    );
...
function openCategoryModal(category = null) {
  ...
  if (category) {
    ...
    document.getElementById("categoryCouleur").value =
      category.couleur || "#7602F9";
    document.getElementById("categoryOrder").value =
      category.ordre_affichage || 1;
...
async function saveCategory() {
  try {
    ...
    const categoryData = {
      ...
      ordre_affichage:
        parseInt(document.getElementById("categoryOrder").value) || 1,
      actif: document.getElementById("categoryActive").checked,
    };

    const url = isEdit
      ? `/admin/admin/api/categories/${categoryId}`
      : "/admin/admin/api/categories";
    const method = isEdit ? "PUT" : "POST";
...
function openProductModal(product = null, preselectedCategoryId = null) {
  ...
  const categorieContainer = document.getElementById(
  ...
  if (product) {
    ...
    document.getElementById("productDescCourte").value =
      product.description_courte || "";
    document.getElementById("productDescLongue").value =
      product.description_longue || "";
    document.getElementById("productCategorie").value =
      product.id_categorie || "";
    document.getElementById("productTag").value = product.tag || "Standard";
    document.getElementById("productStatut").value =
    ...
    document.getElementById("productTypeAchat").value =
      product.type_achat || "panier";
    document.getElementById("productOrder").value =
      product.ordre_affichage || 1;
...
function autoGenerateSlug() {
  ...
  if (nomInput.value && categoryName) {
    ...
    const productSlug = nomInput.value
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, "") // Remove special characters
      .replace(/\s+/g, "-") // Replace spaces with hyphens
      .replace(/-+/g, "-"); // Remove multiple hyphens
  ...
  try {
    ...
    if (response.ok) {
      ...
      const categoryProducts = products.filter(
        (p) =>
          p.id_categorie &&
          p.id_categorie.toString() === currentCategoryId.toString(),
      );
  ...
  try {
    ...
    if (selectedCategory && selectedCategory !== "") {
      products = products.filter(
        (product) =>
          product.id_categorie &&
          product.id_categorie.toString() === selectedCategory,
      );
    }
...
async function saveProduct() {
  try {
    ...
    const productData = {
      ...
      description_courte: document
        .getElementById("productDescCourte")
        .value.trim(),
      description_longue: document
        .getElementById("productDescLongue")
        .value.trim(),
      ...
      ordre_affichage:
        parseInt(document.getElementById("productOrder").value) || 1,
      actif: document.getElementById("productActive").checked,
    };

    const url = productId
      ? `/admin/admin/api/products/${productId}`
      : "/admin/admin/api/products";
  ...
  try {
    ...
    if (select) {
      select.innerHTML =
        ...
        categories
          .map(
            (cat) => `<option value="${cat.id_categorie}">${cat.nom}</option>`,
          )
          .join("");
    }
...
async function moveProduct(productId, direction) {
  try {
    ...
    const response = await fetch(
      `/admin/admin/api/products/${productId}/move`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      loadProducts();
      showAlert(
        `Produit déplacé vers ${direction === "up" ? "le haut" : "le bas"} avec succès!`,
        "success",
      );
    }
  } catch (error) {
...
async function moveCategory(categoryId, direction) {
  try {
    ...
    const response = await fetch(
      `/admin/admin/api/categories/${categoryId}/move`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      loadCategories();
      showAlert(
        `Catégorie déplacée vers ${direction === "up" ? "le haut" : "le bas"} avec succès!`,
        "success",
      );
    }
  } catch (error) {
...
async function moveItemClientSide(type, itemId, direction) {
  try {
    ...
    const orderField =
      type === "product" ? "ordre_affichage" : "ordre_affichage";
    ...
    if (newIndex === currentIndex) {
      showAlert(
        `Le ${type} est déjà à ${direction === "up" ? "la première" : "la dernière"} position`,
        "info",
      );
      return;
    }
    ...
    showAlert(
      `${type === "product" ? "Produit" : "Catégorie"} déplacé${type === "product" ? "" : "e"} avec succès!`,
      "success",
    );
  } catch (error) {
...
async function updateItemOrder(type, itemId, newOrder) {
  ...
  try {
    ...
    if (!listResponse.ok) {
      throw new Error(
        `Erreur lors de la récupération de la liste des ${type}s`,
      );
    }
...
async function loadCategoryProducts() {
  ...
  try {
    ...
    products = products.filter(
      (product) =>
        product.id_categorie &&
        product.id_categorie.toString() === currentCategoryId.toString(),
    );
