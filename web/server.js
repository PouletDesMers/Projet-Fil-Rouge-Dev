...
const {
  ...
  sendNewsletterEmail,
} = require("./backend/js/modules/email");
const {
  generateResetToken,
  saveResetToken,
  validateResetToken,
  markTokenAsUsed,
} = require("./backend/js/modules/reset-tokens");
const app = express();
...
if (STRIPE_SECRET_KEY && STRIPE_SECRET_KEY !== "sk_test_VOTRE_CLE_ICI") {
  try {
    ...
    console.log("[Stripe] SDK chargé");
  } catch (e) {
    console.warn("[Stripe] SDK non disponible:", e.message);
  }
}
...
async function checkAdminAuth(req, res, next) {
  try {
    ...
    // Forward the token to the API to verify user and get profile (avec cache 30s)
    const cached = _getCachedAuth(token);
    let userProfile;
    if (cached === true) {
      // Auth déjà vérifiée récemment, on crée un profil minimal
      userProfile = { role: "admin" };
    } else if (cached === false) {
      return res.redirect("/backend/login.html?error=session_expired");
    } else {
      try {
...
app.post("/auth/login", loginLimiter, async (req, res) => {
  try {
      ...
      if (data.user_id) {
        try {
          const userResponse = await axios.get(
            `http://api:8080/api/users/${data.user_id}`,
            {
    ...
    console.error(
      `[SECURITY] Failed login attempt from IP: ${req.ip}, Error: ${error.message}`,
    );
...
app.post("/api/auth/request-password-reset", async (req, res) => {
  try {
    ...
    const userExists = await axios.get(
      `http://api:8080/api/users/exists?email=${encodeURIComponent(email)}`,
    );
...
app.post("/api/save-verification-token", async (req, res) => {
  try {
    const response = await axios.post(
      "http://api:8080/api/save-verification-token",
      req.body,
    );
    res.json(response.data);
  } catch (error) {
...
app.post("/api/auth/verify-email", async (req, res) => {
  try {
    ...
    const response = await axios.post("http://api:8080/api/verify-email", {
      token,
    });
...
app.post("/api/auth/resend-verification-email", async (req, res) => {
  try {
    ...
    const response = await axios.post(
      "http://api:8080/api/resend-verification-email",
      { email },
    );
...
app.get("/api/public/products/:slug", async (req, res) => {
  try {
    const response = await axios.get(
      `http://api:8080/api/public/products/${req.params.slug}`,
    );
    res.json(response.data);
  } catch (error) {
    res.status(error.response?.status || 500).json({
...
app.get("/api/public/carousel-images", async (req, res) => {
  try {
    const response = await axios.get(
      "http://api:8080/api/public/carousel-images",
    );
    res.json(response.data);
  } catch (error) {
    res.status(error.response?.status || 500).json({
...
app.get("/api/public/search", async (req, res) => {
  try {
    const q = req.query.q || "";
    const response = await axios.get(
      `http://api:8080/api/public/search?q=${encodeURIComponent(q)}`,
    );
    res.json(response.data);
  } catch (error) {
    res.status(error.response?.status || 500).json({
...
app.get("/api/users/exists", async (req, res) => {
  try {
    const response = await axios.get(
      `http://api:8080/api/users/exists?email=${encodeURIComponent(req.query.email)}`,
    );
    res.json(response.data);
  } catch (error) {
    res.status(error.response?.status || 500).json({
...
// Cache d'auth pour éviter de flood l'API Go avec /user/profile
const _authCache = new Map();
const AUTH_CACHE_TTL = 30000; // 30s
...
function _setCachedAuth(token, isAdmin) {
  _authCache.set(token, { isAdmin, expiresAt: Date.now() + AUTH_CACHE_TTL });
}
...
function proxyToApiWithAuth(endpoint) {
  return async (req, res) => {
    try {
      ...
      // Vérifier que l'utilisateur est admin (avec cache 30s)
      const cached = _getCachedAuth(token);
      if (cached === false) {
        return res.status(403).json({ error: "Admin access required" });
      }
      if (cached === null) {
        // Pas en cache → vérifier auprès de l'API
        try {
          const userResponse = await axios.get(
            "http://api:8080/api/user/profile",
            {
              headers: { Authorization: `Bearer ${token}` },
              timeout: 5000,
            },
          ...
          // En cas d'erreur réseau/timeout, on refuse par sécurité
          console.error(`Auth check failed for ${endpoint}:`, authErr.message);
          // Mais on met en cache NÉGATIF 5s pour éviter de re-essayer en boucle
          _setCachedAuth(token, false);
          return res.status(502).json({ error: "Auth service unavailable" });
        }
      }
...
async function runSecurityPentest(req) {
  ...
  const add = (name, passed, details) =>
    checks.push(makePentestCheck(name, passed, details));

  try {
    const health = await axios.get(`${webBase}/health`, {
      timeout: 8000,
      validateStatus: () => true,
    });
    add(
  ...
  try {
    const home = await axios.get(`${webBase}/`, {
      timeout: 8000,
      validateStatus: () => true,
    });
    ...
    for (const header of requiredHeaders) {
      add(
        `Header ${header}`,
        Boolean(home.headers[header]),
        home.headers[header] || "missing",
      );
    }
  } catch (err) {
  ...
  try {
    const swagger = await axios.get(`${apiBase}/swagger`, {
      timeout: 8000,
      validateStatus: () => true,
    });
    add(
      "Swagger endpoint /swagger",
      swagger.status === 200 && Boolean(swagger.data && swagger.data.openapi),
  ...
  try {
    const swaggerJsonResp = await axios.get(`${apiBase}/api/swagger.json`, {
      timeout: 8000,
      validateStatus: () => true,
    });
    swaggerJSON = swaggerJsonResp.data;
    add(
      "Swagger endpoint /api/swagger.json",
      swaggerJsonResp.status === 200 &&
        Boolean(swaggerJSON && swaggerJSON.openapi),
      `status=${swaggerJsonResp.status}`,
    );
  } catch (err) {
  ...
  if (
    ...
    swaggerJSON.components.securitySchemes.BearerAuth
  ) {
    const bearerFormat =
      swaggerJSON.components.securitySchemes.BearerAuth.bearerFormat;
    add(
      "OpenAPI bearerFormat is OpaqueToken",
      bearerFormat === "OpaqueToken",
      String(bearerFormat || "missing"),
    );
  } else {
    add(
      "OpenAPI bearerFormat is OpaqueToken",
  ...
  try {
    ...
    add(
      "Anonymous admin access blocked (web)",
      [401, 403].includes(anonAdmin.status),
  ...
  try {
    const anonApiAdmin = await axios.get(
      `${apiBase}/api/admin/newsletter/subscribers`,
      {
        timeout: 8000,
        validateStatus: () => true,
      },
    ...
    add(
      "Anonymous admin access blocked (api)",
      [401, 403].includes(anonApiAdmin.status),
      `status=${anonApiAdmin.status}`,
    );
  } catch (err) {
  ...
  try {
    ...
    add(
      "CSRF guard blocks cross-origin authenticated write",
      csrfProbe.status === 403,
...
app.post(
  "/admin/api/users/:id/reset-2fa",
  proxyToApiWithAuth("/users/:id/reset-2fa"),
);
app.get(
  "/admin/api/users/:id/roles",
  proxyToApiWithAuth("/admin/users/:id/roles"),
);
app.post(
  "/admin/api/users/:id/roles",
  proxyToApiWithAuth("/admin/users/:id/roles"),
...
app.delete(
  "/admin/api/users/:id/roles/:roleId",
  proxyToApiWithAuth("/admin/users/:id/roles/:roleId"),
);
app.get(
  "/admin/api/users/:id/permissions",
  proxyToApiWithAuth("/admin/users/:id/permissions"),
);
...
app.delete(
  ...
  proxyToApiWithAuth("/web-categories/:id"),
);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const listResponse = await axios.get("http://api:8080/api/web-products", {
        headers,
      });
      const existingProducts = Array.isArray(listResponse.data)
        ? listResponse.data
        : [];

      const keyFor = (slug, idCategorie) =>
        `${String(slug).trim().toLowerCase()}::${idCategorie}`;
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const idCategorie =
          categoryMode === "current" ? forcedCategoryId : rowCategory;
        ...
        try {
          if (mode === "insert") {
            if (existing) {
              skipped++;
              errors.push(
                `Ligne ${line}: produit deja existant pour ce slug/categorie.`,
              );
              continue;
            }

            const createResp = await axios.post(
              "http://api:8080/api/web-products",
              payload,
              { headers },
            );
            created++;
          ...
          if (existing && existing.id_produit) {
            await axios.put(
              `http://api:8080/api/web-products/${existing.id_produit}`,
              {
                ...existing,
                ...payload,
              },
...
app.put(
  "/admin/api/carousel-images/:id",
  proxyToApiWithAuth("/carousel-images/:id"),
);
app.delete(
  "/admin/api/carousel-images/:id",
  proxyToApiWithAuth("/carousel-images/:id"),
...
app.post(
  "/admin/api/carousel-images/reorder",
  proxyToApiWithAuth("/carousel-images/reorder"),
);
...
app.put(
  "/admin/api/api-tokens/:id/status",
  proxyToApiWithAuth("/api-tokens/:id/status"),
);
...
app.get(
  ...
  proxyToApiWithAuth("/admin/stats/top-products"),
);
...
app.get(
  ...
  proxyToApiWithAuth("/admin/roles/:id/permissions"),
);
app.post(
  "/admin/api/roles/:id/permissions",
  proxyToApiWithAuth("/admin/roles/:id/permissions"),
...
app.delete(
  "/admin/api/roles/:id/permissions/:code",
...
app.get(
  ...
  proxyToApiWithAuth("/admin/newsletter/subscribers"),
);
app.get(
...
app.get("/admin/api/quotes", checkAdminAuth, async (req, res) => {
  ...
  try {
    ...
    if (stripe) {
      try {
        const sq = await stripe.quotes.list({
          limit: 100,
    ...
    const enriched = devis.map((c) => {
      const stripeId =
        c.promoCode && c.promoCode.startsWith("qt_") ? c.promoCode : null;
      ...
      if (c.promoCode && !c.promoCode.startsWith("qt_")) {
        try {
          localMeta = JSON.parse(c.promoCode);
        } catch (_) {}
      }
      return {
        id: c.id,
        ...
        stripeQuote: sq
          ? {
              ...
              customer_email:
                sq.metadata?.customerEmail ||
                (typeof sq.customer === "object" ? sq.customer?.email : "") ||
                localMeta.e ||
                "",
              customer_name:
                sq.metadata?.customerName ||
                (typeof sq.customer === "object" ? sq.customer?.name : "") ||
                localMeta.n ||
                "",
...
app.post(
  "/admin/api/quotes/:id/send-stripe",
  checkAdminAuth,
  async (req, res) => {
    const token = getAuthToken(req);
    ...
    if (!email || !productName || !unitPrice) {
      return res
        .status(400)
        .json({ error: "email, productName et unitPrice sont requis" });
    }
...
app.post("/admin/api/discounts", checkAdminAuth, async (req, res) => {
  ...
  if (!code || !type || discount == null) {
    return res
      .status(400)
      .json({ error: "Champs manquants : code, type, discount" });
  }
...
app.get("/api/mes-devis", checkAuth, async (req, res) => {
  try {
    ...
    let stripeMap = {};
    if (stripe) {
      const quoteIds = devis
        .filter((d) => d.promoCode?.startsWith("qt_"))
        .map((d) => d.promoCode);
      for (const qid of quoteIds) {
        try {
          stripeMap[qid] = await stripe.quotes.retrieve(qid, {
            expand: ["line_items"],
          });
        } catch (_) {}
      }
    ...
    const enriched = devis
      .map((d) => {
        let meta = {};
        if (d.promoCode && !d.promoCode.startsWith("qt_")) {
          try {
            meta = JSON.parse(d.promoCode);
          } catch (_) {}
        }
        const sq =
          d.promoCode && stripeMap[d.promoCode] ? stripeMap[d.promoCode] : null;
...
app.get("/admin/api/backup/status", proxyToApiWithAuth("/admin/backup/status"));
...
app.get(
  "/admin/api/backup/schedule",
  proxyToApiWithAuth("/admin/backup/schedule"),
);
app.post(
  "/admin/api/backup/schedule",
  proxyToApiWithAuth("/admin/backup/schedule"),
...
app.get(
  "/admin/api/backup/download",
  proxyToApiWithAuth("/admin/backup/download"),
);
app.post(
  "/admin/api/backup/restore",
  proxyToApiWithAuth("/admin/backup/restore"),
);
...
app.get("/admin/api/swagger.json", checkAdminAuth, async (req, res) => {
  try {
    ...
    spec.servers = [
      { url: "", description: "Via proxy admin (auth par cookie)" },
    ];
...
app.post("/api/user/2fa/setup", checkAuth, async (req, res) => {
  try {
    ...
    const response = await axios.post(
      "http://api:8080/api/user/2fa/setup",
      req.body,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
app.post("/api/user/2fa/verify", checkAuth, async (req, res) => {
  try {
    ...
    const response = await axios.post(
      "http://api:8080/api/user/2fa/verify",
      req.body,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
app.get("/api/public/top-products", async (_req, res) => {
  try {
    ...
    res
      ...
      .json({ error: "Failed to load top products" });
  }
});
...
app.post("/api/validate-promo", async (req, res) => {
  ...
  const localMatch = DEMO_PROMO_CODES.find(
  ...
  try {
    ...
    if (!promos.data.length) {
      return res
        .status(400)
        .json({ valid: false, error: "Code promo invalide ou expiré" });
    }
...
app.post("/api/request-quote", async (req, res) => {
  ...
  if (!userId) {
    return res.status(401).json({
      error: "Vous devez être connecté pour soumettre une demande de devis.",
    });
  }
...
app.post("/api/create-checkout-session", checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (promoCode) {
      ...
      const localCode = DEMO_PROMO_CODES.find(
        (c) => c.code === promoCode.toUpperCase() && c.active,
      );
      if (localCode) {
        const couponParams =
          localCode.type === "percent"
            ? {
                percent_off: localCode.discount,
                duration: "once",
                name: localCode.code,
              }
            : {
                amount_off: Math.round(localCode.discount * 100),
                currency: "eur",
                duration: "once",
                name: localCode.code,
              };
...
app.post("/api/confirm-order", checkAuth, async (req, res) => {
  ...
  if (sessionId && stripe && !demo) {
    try {
      ...
      console.warn(
        "[confirm-order] Impossible de récupérer la session Stripe:",
        e.message,
      );
    }
  }
    ...
    const normalized = Array.isArray(items)
      ? items.map((it) => ({
          product_slug:
            ...
            "",
          product_name:
            it.product_name || it.productName || it.name || "Produit",
...
app.post("/api/test/send-email", async (req, res) => {
  ...
  try {
    ...
    if (type === "welcome") {
      ...
      return res
        .status(400)
        .json({ error: "Invalid email type (welcome, reset, admin)" });
    }
