...
async function loadCategoryInfo() {
  try {
    ...
    document.getElementById("categoryTitle").textContent =
      `Catalogue ${currentCategory.toUpperCase()}`;
    document.getElementById("categoryDescription").textContent =
      "Services de cybersécurité.";
    document.getElementById("pageTitle").textContent =
      `Catalogue ${currentCategory.toUpperCase()} | CYNA`;
  }
}
...
function updateCategoryUI(category) {
  ...
  document.getElementById("categoryTitle").textContent =
    `Catalogue ${category.nom}`;
  document.getElementById("categoryDescription").textContent =
    category.description;
  document.getElementById("pageTitle").textContent =
    `Catalogue ${category.nom} | CYNA`;
...
function renderProductCard(product) {
  ...
  const actionButton =
    product.type_achat === "devis"
      ...
      <button data-id="${product.slug}" data-name="${(product.nom || "").replace(/"/g, "&quot;")}" data-price="${product.prix || 0}" data-duration="${product.duree || "mois"}" class="btn-add-to-cart btn-cyna btn-action">
...
function getFirstImage(raw) {
  ...
  for (const img of images) {
    ...
    if (typeof img === "string") url = img;
      ...
      url = img.url || img.url_image || img.src || "";
...
function requestQuote(productName) {
  showToast(
    "info",
    `Demande de devis pour ${productName} en cours de traitement...\\n\\nUn email sera envoyé pour confirmer.`,
  );
}
